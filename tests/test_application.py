...
from wheelguard.models import ProjectRepository, ProjectResponse, UpstreamNotFoundError
...
def make_app(repository: ProjectRepository, data_dir: Path) -> FastAPI:
    return create_app(
        settings=Settings(minimum_age=timedelta(days=14), data_dir=data_dir),
